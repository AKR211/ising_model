...
@njit(cache=True)
def main(size,betaJ,n,p=0.5,B=0):
	Magnetization=[]
	Energy=[]                 
	...
	for k in range(n):
		...
		dE=2*interaction(L,i,j)+2*B*L[i,j]  #change in energy
...
@njit(cache=True)
def data1(size,betaJ,n,p=0.5,B=0):
	Magnetization=[]
	Energy=[]                 
	L=lattice_builder(size,p)
	U=energy(L,B)
...
if __name__=='__main__':
	...
	#plotting
	Energy,Magnetization,Lattice_init,Lattice,History=main(size,1/Temp,n,p,B)
